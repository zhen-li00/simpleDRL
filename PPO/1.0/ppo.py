...
class Agent(object):
    def __init__(self, **kwargs):
        ...
        # 构建Critic网络
        self.critic = Critic(s_dim, 256, 1)
    ...
    def choose_action(self, s0):        #
        s0 = torch.tensor(s0, dtype=torch.float).unsqueeze(0)   # 升维度，（可能涉及维度问题）

        dist = self.actor(s0)   # 通过actor网络得到动作的概率分布
        # action = torch.squeeze(dist.sample(), 0)[0]
        action = dist.sample()  # 从该正态分布中采样得到一个action
        probs = torch.squeeze(dist.log_prob(action)).detach().numpy() # log_prob()函数，计算这个action的log概率，
                                                                         # 构造等效的损失函数，用于求策略梯度
    ...
    def update(self):
        ...
        for _ in range(self.n_epochs):      # n_epochs?
            ...
            for batch in batches:
                ...
                weighted_probs = torch.matmul(advantage[batch], prob_ratio)
                # weighted_probs = advantage[batch] * prob_ratio
                # Clipping the probability ratio，裁剪概率比例
                weighted_clipped_probs = torch.matmul(advantage[batch],
